...
This repo is an NVChad overlay. Custom config and plugins live in `lua/custom` and are copied into `~/.config/nvim/lua/custom` by the installer (currently includes neoscroll, VimBeGood, Python debugging via nvim-dap, and nvim-tree showing gitignored/hidden files by default). A custom theme named `hacker` is installed from `lua/themes/hacker.lua` (Catppuccin palette with pure black backgrounds; all colors are listed there for manual edits). A small compatibility shim is also installed at `lua/chadrc.lua`, and for newer NvChad starters the installer adds an import in `lua/plugins/init.lua` to load `lua/plugins/custom.lua`.
